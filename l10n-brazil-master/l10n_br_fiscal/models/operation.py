...
class Operation(models.Model):
    ...
    states_destination = fields.Many2many(
        comodel_name="res.country.state",
        string="Lista de estados Destinos",
    ...
    company_id = fields.Many2one(
        ...
        default=lambda self: self.env["res.company"].browse(
            self.env['res.company']._company_default_get('l10n_br_fiscal.operation')),
        readonly=True,
        states={"draft": [("readonly", False)]},
        tracking=True,
    )
    ...
    def create(self, vals_list):
        operation_list = self.search([])
        for operation in operation_list:
            for states_destination_code in operation.states_destination:
                if states_destination_code.id in vals_list['states_destination'][0][2]:
                    raise UserError(_("A record already has that state."))
        return super(Operation, self).create(vals_list)

    def write(self, values):
        operation_list = self.search([('id', '!=', self.id)])
        for operation in operation_list:
            if 'states_destination' not in values:
                continue
            for states_destination_code in operation.states_destination:
                if states_destination_code.id in values['states_destination'][0][2]:
                    raise UserError(_("A record already has that state."))
        return super(Operation, self).write(values)
