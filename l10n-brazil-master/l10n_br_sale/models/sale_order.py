...
class SaleOrder(models.Model):
    ...
    def onchange_partner_id_to_operation(self):
        if self.partner_id:
            self.fiscal_operation_id = self.env['l10n_br_fiscal.operation'].search(
                [('states_destination', 'in', self.partner_id.state_id.id)])
            if not self.fiscal_operation_id:
                raise UserError("The given client has no recipient state related to it.")
