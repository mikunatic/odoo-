...
class SaleOrderLine(models.Model):
    ...
    def _fiscal_operation_domain(self):
        domain = [("state", "=", "approved"), ("states_destination", "in", self.partner_id.state_id.id)]
        return domain
    ...
    def _product_ncm_domain(self):
        self.fiscal_operation_line_id = self.env['l10n_br_fiscal.operation.line'].search(
            [('fiscal_operation_id', '=', self.fiscal_operation_id.id),
             ('state', '=', 'approved'),
             ('ncm_ids', 'in', [self.product_id.ncm_id.id]),
             ('icms_origin', '=', self.product_id.icms_origin),
             ('product_type', '=', self.product_id.fiscal_type)])
        if self.product_id:
            cfop_obj = self.env['l10n_br_fiscal.operation.line'].browse(self.fiscal_operation_line_id.id)
            if self.env.company.state_id.id.__eq__(self.partner_id.state_id.id):
                cfop_id = cfop_obj.cfop_internal_id.id
            elif not self.env.company.state_id.id.__eq__(
                self.partner_id.state_id.id) and self.env.company.country_id.id.__eq__(
                self.partner_id.country_id.id):
                cfop_id = cfop_obj.cfop_external_id.id
            else:
                cfop_id = cfop_obj.cfop_export_id.id
            if cfop_id:
                self.cfop_id = cfop_id
            else:
                raise UserError(
                    _("O produto selecionado não possui uma configuração de acordo com os CFOP's da operação em questão."))
