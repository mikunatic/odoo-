...
class AccountTax(models.Model):
    ...
    def compute_all(
        ...
        for account_tax in taxes_results["taxes"]:
            ...
            if fiscal_tax:
                if fiscal_tax.get('tax_domain').__eq__('icmsst'):
                    value_total = fiscal_tax.get('base') * (fiscal_tax.get('percent_amount') / 100)
                    for rec in taxes_results['taxes']:
                        if rec['name'].__eq__("ICMS"):
                            value_total -= rec['amount']
                    fiscal_tax['tax_value'] = -1 * value_total

                if fiscal_tax.get("base") < 0:
                    sign = -1
                    fiscal_tax["base"] = -fiscal_tax.get("base")

                if not fiscal_tax.get("tax_include") and not tax.deductible or fiscal_tax.get('tax_domain').__eq__(
                    'icmsfcp'):
